 ...
 */
//HI_MPR121::HI_MPR121(byte _SDA, byte _SCL) :
//  SDA(_SDA),
//  SCL(_SCL),
//  defaultI2C( 0 )
//  {}
 ...
 */
boolean HI_MPR121::begin(uint8_t i2caddr, TwoWire *theWire,
                               uint8_t touchThreshold,
                               uint8_t releaseThreshold) {
  _i2caddr = i2caddr;
  _wire = theWire;

  if(defaultI2C) _wire->begin();
  else  _wire->begin();
...
//  writeRegister(MPR121_ECR, ECR_SETTING); // start with above ECR setting
  byte ECR_SETTING =
      B10110000 + num_electrodes; // 2 bits for baseline tracking, 2 for proximity enabled
                      //  + amount of electrodes running (12)
...
/*!
 *  @brief      DEPRECATED. Use HI_MPR121::setThresholds(uint8_t touch,
 *              uint8_t release) instead.
 *  @param      touch
 *              see HI_MPR121::setThresholds(uint8_t touch, uint8_t
 * *release)
 *  @param      release
 *              see HI_MPR121::setThresholds(uint8_t touch, *uint8_t
 * release)
 */
void HI_MPR121::setThreshholds(uint8_t touch, uint8_t release) {
 ...
 */
void HI_MPR121::setThresholds(uint8_t touch, uint8_t release) {
  // first stop sensor to make changes
  uint16_t curVal = readRegister16(MPR121_ECR);
  ...
  writeRegister(MPR121_ECR, curVal);
}
 ...
 */
uint16_t HI_MPR121::filteredData(uint8_t t) {
  if (t > 12)
    return 0;
 ...
 */
uint16_t HI_MPR121::baselineData(uint8_t t) {
   if (t > 12)
     return 0;
...
uint16_t HI_MPR121::proximityData() {
 ...
 */
uint16_t HI_MPR121::touched(void) {
 ...
 */
uint8_t HI_MPR121::readRegister8(uint8_t reg) {
 ...
 */
uint16_t HI_MPR121::readRegister16(uint8_t reg) {
  _wire->beginTransmission(_i2caddr);
  _wire->write(reg);
  _wire->endTransmission(false);
...
*/
void HI_MPR121::writeRegister(uint8_t reg, uint8_t value) {
...
void HI_MPR121::chargeCurrent(byte val){
 byte FFI = 0;
 val > 63 ? val = 63 : 0;
 val = (FFI<<6) + val;
...
void HI_MPR121::chargeTime(byte val){
 byte SFI = 0;
 byte ESI = 0;
 val < 1 ? val=1: 0;
 val > 7 ? val = 7 : 0;
...
void HI_MPR121::proxChargeCurrent(byte val){
 val > 63 ? val = 63 : 0;
 writeRegister(MPR121_PROXIMITY_CHARGECURRENT, val);
}

void HI_MPR121::proxChargeTime(byte val){
 val > 7 ? val = 7 : 0;
...
/*enableGPIO
 * 
 * GPIO 0-7 are on electrodes 4-11
 * 
 * 3-bit state possibilities:
 * - bit 0: input(0) or output(1)
 * - bit 1&2: input enables pullup or down resistors
 * - bit 1&2: output 00 CMOS output port
 *                   11 high side open drain LED driver
 *                   10 low side open drain output port
 *  
 *  For LED driver set state to 111 / byte 7
 */
 void HI_MPR121::enableGPIO(byte num, byte state){
  if(num > 7) return 0;

  byte enable = readRegister8(MPR121_GPIOEN);
  if(1) enable |= 1<<num; //if we are enabling a pin
  else enable &= (255-(1<<num));
  writeRegister(MPR121_GPIOEN, enable);

  byte direct = readRegister8(MPR121_GPIODIR);
  if(state&4) direct |= 1<<num; //if we are enabling a pin
  else direct &= (255-(1<<num));
  writeRegister(MPR121_GPIODIR, direct);

  byte ctl0 = readRegister8(MPR121_GPIO_CTL0);
  if(state&2) ctl0 |= 1<<num; //if we are setting a bit
  else ctl0 &= (255-(1<<num));
  writeRegister(MPR121_GPIO_CTL0, ctl0);

  byte ctl1 = readRegister8(MPR121_GPIO_CTL1);
  if(state&1) ctl1 |= 1<<num; //if we are setting a bit
  else ctl1 &= (255-(1<<num));
  writeRegister(MPR121_GPIO_CTL1, ctl1);
}

 void HI_MPR121::disableGPIO(byte num){
  if(num > 7) return 0;

  byte enable = readRegister8(MPR121_GPIOEN);
  if(0) enable |= 1<<num; //if we are enabling a pin
  else enable &= (255-(1<<num));
  writeRegister(MPR121_GPIOEN, enable);
 }

  void HI_MPR121::setGPIO(byte num, byte val){
  if(num > 7) return 0;

  if(val > 0) GPIO_DATA |= 1<<num; //if we are setting a pin
  else GPIO_DATA &= (255-(1<<num));
  writeRegister(MPR121_GPIO_DATA, GPIO_DATA);
 }

 void HI_MPR121::updateGPIO(){ writeRegister(MPR121_GPIO_DATA, GPIO_DATA); }

 void HI_MPR121::monitorCapSettings(){
  Serial.println("enable " + String(readRegister8(MPR121_GPIOEN), BIN));
  Serial.println("dir " + String(readRegister8(MPR121_GPIODIR), BIN));
  Serial.println("ctl0 " + String(readRegister8(MPR121_GPIO_CTL0),BIN));
  Serial.println("ctl1 " + String(readRegister8(MPR121_GPIO_CTL1),BIN));
  Serial.println("ECR " + String(readRegister8(MPR121_ECR),BIN));
  Serial.println("DATA " + String(GPIO_DATA,BIN) + " " + String(readRegister8(MPR121_GPIO_DATA),BIN));
  Serial.println();
 }
