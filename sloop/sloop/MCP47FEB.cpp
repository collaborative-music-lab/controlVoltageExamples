/*
  ...
  mcp47FEB.cpp - Arduino library for MicroChip mcp47FEB I2C D/A converter
  ...
  https://github.com/akuma6099/MCP47FEB 
*/
...
*/
mcp47FEB::mcp47FEB(uint8_t deviceID = 0x00)
{
  _deviceID = deviceID;
...
/*
Begin I2C, get current values (input register and eeprom) of mcp47FEB
*/
void mcp47FEB::begin()
{
  Wire.begin();
...
/*
General Call Reset of mcp47FEB - EEPROM values will be loaded into the input registers.
*/
uint8_t mcp47FEB::reset() {
  return _simpleCommand(RESET);
}
/*
General Call Wake-Up of mcp47FEB - Reset Power-Down bits (PD0,PD1 = 0,0). refer to DATASHEET 5.4.2
*/
uint8_t mcp47FEB::wake() {
...
*/
uint8_t mcp47FEB::analogWrite(uint16_t value1, uint16_t value2) {
...
uint8_t mcp47FEB::setVref(uint8_t value1, uint8_t value2) {
  _intVref[0] = value1;
  _intVref[1] = value2 << 2;
  return writeVref();
}
/* 
  Write Gain settings to input regiters
    ...
    Vref setting = 0 (external), Gain = ignored  ==> Vref = VDD
*/
uint8_t mcp47FEB::setGain(uint8_t value1, uint8_t value2) {
  _gain[0] = value1;
  _gain[1] = value2 << 1;
  return writeGain();
}
...
uint8_t mcp47FEB::setPowerDown(uint8_t value1, uint8_t value2) {
  _powerDown[0] = value1;
...
*/
uint8_t mcp47FEB::getVref(uint8_t channel)
{
    switch(channel)
    {
        case 0:
...
/*
  Return Gain setting
*/
uint8_t mcp47FEB::getGain(uint8_t channel)
{
    switch(channel)
    {
        case 0:
...
*/
uint8_t mcp47FEB::getPowerDown(uint8_t channel)
{
    switch(channel)
    {
        case 0:
...
*/
uint16_t mcp47FEB::getValue(uint8_t channel)
{
...
*/
uint8_t mcp47FEB::getVrefEp(uint8_t channel)
{
    switch(channel)
    {
        case 0:
...
*/
uint8_t mcp47FEB::getGainEp(uint8_t channel)
{
    switch(channel)
    {
        case 0:
...
*/
uint8_t mcp47FEB::getPowerDownEp(uint8_t channel)
{
    switch(channel)
    {
        case 0:
...
/*
  Return EEPROM value
*/
uint16_t mcp47FEB::getValueEp(uint8_t channel)
{
...
*/
void mcp47FEB::vdd(uint16_t currentVdd)
{
  _vdd = currentVdd;
}
...
/*
Get current values (input register and eeprom) of mcp47FEB
*/
void mcp47FEB::getStatus()
{
    //Volatile Registers
...
FastWrite
*/
uint8_t mcp47FEB::fastWrite(uint8_t reg, uint16_t value) {
  Wire.beginTransmission(_dev_address);
...
No EEPROM update
*/
uint8_t mcp47FEB::writePowerDown() {
  Wire.beginTransmission(_dev_address);
...
*/
uint8_t mcp47FEB::_simpleCommand(byte simpleCommand) {
