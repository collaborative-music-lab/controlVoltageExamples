...
void handleCtlY(byte num, byte val){
  if( launchpad_page == 2) {
    if(num>3) {
      update_scale_leds(_cur_scale,0);
      byte _cur_scale = num-4;
      update_scale_leds(_cur_scale,16);
    }
    else byte _cur_octave = num + 0;
    return 0;
  }
  lp.ctlY[num].val = val;
...
void handleGrid(byte num, byte val){
  ...
  /*below for working with four rhythm tracks*/
    ...
    //check if a pattern select button is held down
    if( lp.ctlY[patternSelect+4].val > 0) {
      params.seqTiming[patternSelect] = num*4;
      Serial.println("timing " + String(patternSelect) + " " + String(params.seqTiming[patternSelect]));
      return 0;
    }
...
void handleCVseq(byte num){
  

  if(num < 56  ){ //enter note
    byte _cur_step = num%8; 
    byte prev_note = _melody[_active_melody][_cur_step];
    updateGridLed( (prev_note)*8 + _cur_step,  0,0);
    _melody[_active_melody][_cur_step] = num/8;
    updateGridLed( (_melody[_active_melody][_cur_step]) * 8 + _cur_step, 1, 16);
  }
...
void processCC(byte num, byte val) {
  ...
  if(num < 4){
    ...
    Serial.println("page " + String(launchpad_page));
    //updateGrid();
