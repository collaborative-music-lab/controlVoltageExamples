...
byte getDivide(byte chan, byte num){
  char val = 0;
  if(globalDivide==0) val = chan==0 ? seqDivide[num] : seqChanBDivide[num];
  else {
    if( chan == 0 ) val = globalDivide == 1 ? seqDivide[num]/2 : seqDivide[num]*2;
    else val = globalDivide == 1 ? seqChanBDivide[num]/2 : seqChanBDivide[num]*2;
  }
  val = val > 0 ? val :  1;
  return val;
}
...
void setDivideTarget(byte _drum, byte chan){
  ...
  if( chan == 0 ) lcd.write(  seqDivide[_drum] + 48 );
...
void printArray(int *array, byte size){
  if( SERIAL_DEBUG ) Serial.print("sequence: ");
